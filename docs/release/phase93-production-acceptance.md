...
| 5 | backup / restore | **VERIFIED (mechanism)** | `backup`/`verify`/`restore` scripts; **Phase 93 adds a real CI backup→restore rehearsal** with per-table row-count integrity + RTO. Accepted gaps: no at-rest encryption, no restore env-guard (DR runbook §2). |
