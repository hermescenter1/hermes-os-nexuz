// Build + package the Hermes Phase 102 Media Hub Builder plugin.
//
...
// dist/code.js using a tiny inlined CommonJS runtime. It also copies the UI
// and writes an auditable build report. With --dry-run it additionally
// executes the PURE spec/plan in Node (no `figma`) and prints exactly what an
// in-Figma Dry Run would enumerate.
//
...
function bundle() {
  const parts = []
  parts.push('/* Hermes Phase 102 Media Hub Builder — generated bundle. Do not edit dist/ by hand; edit src/ and run `npm run build`. */')
  parts.push('(function () {')
...
function main() {
  ...
  const { planPages } = require('./src/lib/pages.js')
  const C = require('./src/lib/constants.js')
  const spec = buildSpec()
  const plan = computePlan(spec, {}) // empty index == fresh file == all "create"
  const freshPagePlan = planPages([], C.PAGE_NAMES) // no pre-existing pages
  const typicalPagePlan = planPages(['Page 1'], C.PAGE_NAMES) // the file's real starting state

  const report = {
    plugin: 'Hermes Phase 102 Media Hub Builder',
    ...
    freshFilePagePlan: freshPagePlan,
    typicalPagePlan,
  ...
  if (dryRun) {
    ...
    console.log('\nPage plan (typical: file starts with a single "Page 1"):')
    console.log('  ' + typicalPagePlan.actions.map((a) => a.action + ' → ' + a.name + (a.fromName ? ' (from "' + a.fromName + '")' : '')).join('\n  '))
