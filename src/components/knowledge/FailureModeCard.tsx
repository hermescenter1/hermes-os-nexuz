  ...
  return (
    <GlassCard className="p-4 space-y-2">
      <div className="flex items-start justify-between gap-2">
        <p className="font-semibold text-ink text-sm leading-tight" dir="auto">{fm.name}</p>
      ...
      {fm.description && (
        <p className="text-ink/40 text-xs leading-relaxed" dir="auto">{fm.description}</p>
      )}

      {fm.symptoms && fm.symptoms.length > 0 && (
        <div className="flex flex-wrap gap-1 pt-1">
          {fm.symptoms.slice(0, 5).map((s, i) => (
            <span key={i} className="text-xs bg-white/5 border border-white/10 rounded px-1.5 py-0.5 text-ink/40" dir="auto">
              {s}
            </span>
          ))}
          {fm.symptoms.length > 5 && (
            <span className="text-xs text-ink/20">+{fm.symptoms.length - 5}</span>
          )}
        </div>
      )}
