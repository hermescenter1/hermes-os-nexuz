  ...
  return (
    <GlassCard className="p-4 space-y-2">
      <div className="flex items-start justify-between gap-2">
        <p className="font-semibold text-ink text-sm leading-tight" dir="auto">{c.title}</p>
      ...
      {c.symptoms && c.symptoms.length > 0 && (
        <div className="flex flex-wrap gap-1">
          ...
          {c.symptoms.length > 4 && (
            <span className="text-xs text-ink/20">+{c.symptoms.length - 4}</span>
          )}
        </div>
      )}

      {c.diagnosis && (
        <p className="text-ink/40 text-xs leading-relaxed" dir="auto">{c.diagnosis}</p>
      )}
