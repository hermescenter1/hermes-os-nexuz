  ...
  return (
    <GlassCard className="p-4 space-y-2">
      <div className="flex items-start justify-between gap-2">
        <p className="font-semibold text-white text-sm leading-tight" dir="auto">
          {article.title}
        </p>
      ...
      {article.summary && (
        <p className="text-white/40 text-xs leading-relaxed" dir="auto">{article.summary}</p>
      )}

      <div className="flex items-center gap-3 pt-1">
        <span className="font-mono text-xs text-white/25">v{article.version}</span>
        {article.sourceType && (
          <span className="text-xs text-white/25">{article.sourceType.replace(/_/g, " ")}</span>
        )}
        {article.updatedAt && (
