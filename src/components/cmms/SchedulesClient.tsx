  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-metadata font-mono">({schedules.length})</span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-line bg-surface2">
              <th className="text-start px-4 py-3 text-xs font-semibold text-metadata uppercase tracking-wide">{t("schedules.colName")}</th>
              <th className="text-start px-4 py-3 text-xs font-semibold text-metadata uppercase tracking-wide">{t("schedules.colStatus")}</th>
              <th className="text-start px-4 py-3 text-xs font-semibold text-metadata uppercase tracking-wide hidden md:table-cell">{t("schedules.colPriority")}</th>
              <th className="text-start px-4 py-3 text-xs font-semibold text-metadata uppercase tracking-wide">{t("schedules.colDate")}</th>
              <th className="text-start px-4 py-3 text-xs font-semibold text-metadata uppercase tracking-wide hidden lg:table-cell">{t("schedules.colHours")}</th>
              <th className="text-start px-4 py-3 text-xs font-semibold text-metadata uppercase tracking-wide hidden lg:table-cell">{t("schedules.colTechnician")}</th>
              <th className="text-start px-4 py-3 text-xs font-semibold text-metadata uppercase tracking-wide hidden xl:table-cell">{t("schedules.colAsset")}</th>
            </tr>
          </thead>
                ...
                return (
                  <tr key={s.id} className="hover:bg-surface2 transition-colors">
                    ...
                    <td className="px-4 py-3 hidden xl:table-cell">
                      <span className="text-xs font-mono text-metadata">{s.assetId ?? "—"}</span>
                    </td>
                  </tr>
                );
              })
