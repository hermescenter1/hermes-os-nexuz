    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-line bg-surface p-6">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <Link href="/customer/projects" className="text-xs text-metadata hover:text-signal">← Projects</Link>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-metadata">Overall Progress</span>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
          <div><p className="text-xs text-metadata">Manager</p><p className="text-ink">{project.managerName ?? "—"}</p></div>
          <div><p className="text-xs text-metadata">Priority</p><p className="text-ink">{project.priority}</p></div>
          <div><p className="text-xs text-metadata">Start Date</p><p className="text-ink">{project.startDate ? formatDate(project.startDate, locale) : "—"}</p></div>
          <div><p className="text-xs text-metadata">End Date</p><p className="text-ink">{project.endDate ? formatDate(project.endDate, locale) : "—"}</p></div>
        </div>
      </div>
      ...
      {project.teamMembers.length > 0 && (
        <div className="rounded-xl border border-line bg-surface p-6">
          <p className="font-mono text-xs uppercase tracking-widest text-metadata mb-3">Team</p>
      ...
      <div className="rounded-xl border border-line bg-surface">
        <div className="px-6 py-4 border-b border-line">
          <p className="font-mono text-xs uppercase tracking-widest text-metadata">Milestones</p>
        </div>
        {milestones.length === 0 ? (
            ...
            {milestones.map((ms) => (
              <li key={ms.id} className="px-6 py-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className={`text-sm ${ms.completed ? "line-through text-metadata" : "text-ink"}`}>{ms.title}</p>
                    {ms.description && <p className="text-xs text-muted">{ms.description}</p>}
                  </div>
                </div>
                {ms.dueDate && (
                  <span className="shrink-0 text-xs text-metadata">{formatDate(ms.dueDate, locale)}</span>
                )}
              </li>
            ))}
          </ul>
      ...
      {project.tags.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {project.tags.map((t) => (
            <span key={t} className="rounded-full border border-line px-3 py-1 text-xs text-metadata">{t}</span>
          ))}
        </div>
      )}
    </div>
