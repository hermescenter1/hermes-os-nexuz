    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="grid sm:grid-cols-3 gap-4 text-sm">
        <div className="rounded-xl border bg-card p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("templateDetail.category")}</div>
          <div className="font-medium">{template.category}</div>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("templateDetail.trigger")}</div>
          <div className="font-mono text-xs">{template.triggerType}</div>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("templateDetail.usage")}</div>
          <div className="font-medium">{t("templateDetail.usageCount", { count: template.usageCount })}</div>
        </div>
      </div>

      <div className="rounded-xl border bg-card p-4 space-y-3">
        <h3 className="font-semibold">{t("templateDetail.conditions", { count: template.definition.conditions.length })}</h3>
        {template.definition.conditions.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("templateDetail.alwaysExecutes")}</p>
        ) : (
          template.definition.conditions.map((c, i) => (
            <div key={i} className="text-sm flex items-center gap-3">
              ...
              {c.value && <span className="text-muted-foreground">{t("templateDetail.valuePrefix")} {c.value}</span>}
            </div>
          ))
        )}
      </div>
      ...
      <div className="rounded-xl border bg-card p-4 space-y-3">
        <h3 className="font-semibold">{t("templateDetail.actions", { count: template.definition.actions.length })}</h3>
