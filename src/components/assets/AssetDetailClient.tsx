    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-surface rounded-xl p-5">
          <p className="eyebrow-label text-metadata mb-4">{t("detail.technicalProfile")}</p>
          <div>
          ...
          {Object.keys(asset.technicalSpecs).length > 0 && (
            <div className="mt-4 border-t border-line pt-4">
              <p className="text-xs text-metadata mb-2">{t("detail.specifications")}</p>
              {Object.entries(asset.technicalSpecs).map(([k, v]) => (
                <div key={k} className="grid grid-cols-[140px_1fr] gap-2 py-1 border-b border-line/30">
                  <span className="text-xs text-metadata">{k}</span>
        ...
        <div className="card-surface rounded-xl p-5">
          <p className="eyebrow-label text-metadata mb-4">{t("detail.latestHealthSnapshot")}</p>
          {latestHealth ? (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                {latestHealth.vibrationRms !== null && (
                  <div className="bg-surface2 rounded-lg p-3">
                    <p className="text-xs text-metadata">{t("detail.vibrationRms")}</p>
                    <p className="text-lg font-semibold text-ink">{latestHealth.vibrationRms?.toFixed(1)}<span className="text-xs text-metadata ms-1">mm/s</span></p>
                  </div>
                )}
                {latestHealth.temperature !== null && (
                  <div className="bg-surface2 rounded-lg p-3">
                    <p className="text-xs text-metadata">{t("detail.temperature")}</p>
                    <p className="text-lg font-semibold text-ink">{latestHealth.temperature?.toFixed(0)}<span className="text-xs text-metadata ms-1">°C</span></p>
                  </div>
                )}
                {latestHealth.pressure !== null && (
                  <div className="bg-surface2 rounded-lg p-3">
                    <p className="text-xs text-metadata">{t("detail.pressure")}</p>
                    <p className="text-lg font-semibold text-ink">{latestHealth.pressure?.toFixed(1)}<span className="text-xs text-metadata ms-1">bar</span></p>
                  </div>
                )}
                {latestHealth.currentDraw !== null && (
                  <div className="bg-surface2 rounded-lg p-3">
                    <p className="text-xs text-metadata">{t("detail.currentDraw")}</p>
                    <p className="text-lg font-semibold text-ink">{latestHealth.currentDraw?.toFixed(0)}<span className="text-xs text-metadata ms-1">A</span></p>
                  </div>
                )}
              </div>
              {latestHealth.notes && <p className="text-xs text-metadata border-t border-line pt-3">{latestHealth.notes}</p>}
              <p className="text-xs text-metadata">{formatDate(latestHealth.takenAt, locale)}</p>
            </div>
          ) : (
          ...
          {latestCrit && (
            <div className="mt-5 border-t border-line pt-5">
              <p className="eyebrow-label text-metadata mb-3">{t("detail.criticalityAssessment")}</p>
              ...
              {latestCrit.notes && <p className="text-xs text-metadata">{latestCrit.notes}</p>}
            </div>
          )}
        </div>
      </div>
      ...
      {asset.lifecycleEvents.length > 0 && (
        <div className="card-surface rounded-xl p-5">
          <p className="eyebrow-label text-metadata mb-4">{t("detail.lifecycleHistory")}</p>
          <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card-surface rounded-xl p-4">
          <p className="eyebrow-label text-metadata mb-3">{t("detail.maintenance")}</p>
          {asset.maintenanceLinks.length === 0 ? (
            <p className="text-xs text-metadata">{t("detail.noWorkOrders")}</p>
          ) : (
            <div className="space-y-2">
              {asset.maintenanceLinks.map(lnk => (
                <div key={lnk.id} className="text-xs">
                  ...
                  {lnk.notes && <p className="text-metadata mt-0.5">{lnk.notes}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        {/* Documents */}
        <div className="card-surface rounded-xl p-4">
          <p className="eyebrow-label text-metadata mb-3">{t("detail.documents")}</p>
          {asset.documentLinks.length === 0 ? (
            <p className="text-xs text-metadata">{t("detail.noDocuments")}</p>
          ) : (
            <div className="space-y-2">
              {asset.documentLinks.map(doc => (
                <div key={doc.id} className="text-xs">
                  <p className="text-ink font-medium">{doc.title}</p>
                  <p className="text-metadata">{doc.docType}</p>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Telemetry */}
        <div className="card-surface rounded-xl p-4">
          <p className="eyebrow-label text-metadata mb-3">{t("detail.telemetry")}</p>
          {asset.telemetryLinks.length === 0 ? (
            <p className="text-xs text-metadata">{t("detail.noTags")}</p>
          ) : (
            <div className="space-y-2">
              {asset.telemetryLinks.map(tl => (
                <div key={tl.id} className="text-xs">
                  <p className="font-mono text-ice text-xs">{tl.tagPath}</p>
                  <p className="text-metadata">{tl.protocol}{tl.unit ? ` · ${tl.unit}` : ""}</p>
                </div>
              ))}
            </div>
          )}
      ...
      {asset.assetTags.length > 0 && (
        <div className="card-surface rounded-xl p-4">
          <p className="eyebrow-label text-metadata mb-3">{t("detail.tags")}</p>
          <div className="flex flex-wrap gap-2">
            {asset.assetTags.map(tag => (
              <div key={tag.id} className="flex items-center gap-1 bg-surface2 border border-line/50 rounded px-2 py-1">
                <span className="text-xs text-metadata">{tag.key}:</span>
