    ...
    <GlassCard title={t("graphView")}>
      <div className="space-y-4">
        ...
        <div className="flex gap-6 text-sm">
          <div>
            <span className="text-ink/40">{t("nodes")}:</span>{" "}
            <span className="text-ink font-medium">{nodeList.length}</span>
          </div>
          <div>
            <span className="text-ink/40">{t("relations")}:</span>{" "}
            <span className="text-ink font-medium">{relations.length}</span>
          </div>
        </div>
        ...
        <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
          {nodeList.map((n) => (
            <div key={n.id} className="flex items-center gap-3 py-1.5 px-2 rounded hover:bg-white/5">
              ...
              <span className="text-ink/80 text-sm truncate">{n.displayName}</span>
              <span className="text-ink/30 text-xs ml-auto">{n.nodeType}</span>
              {n.children.length > 0 && (
                <span className="text-ink/30 text-xs">{n.children.length} {t("children")}</span>
              )}
            </div>
          ))}
        </div>
