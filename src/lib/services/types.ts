 ...
 * implementation. Today they are backed by in-process logic and Next.js API
 * routes (the BFF).
 *
 * FUTURE WORK — not shipped, not connected, not scheduled here: an
 * authenticated acquisition path (AI Gateway, OPC UA / Modbus TCP / MQTT
 * gateways, Historian, Auth, Audit) could be introduced behind these same
 * interfaces. No such gateway, historian, protocol driver or plant connection
...
// PHASE 109-B0 — the `TelemetryService` interface that used to sit here was
// retired together with `/api/telemetry` and `telemetry-service.ts`. It
// described a service that fetched an anonymous route serving plant-shaped
// simulated values; the dashboard now reads a classified frame from the
// isolated local demo adapter in `src/lib/dashboard-demo` instead, and no
// service layer stands between them.
//
// `TelemetryPoint` is a pre-existing exported shape and is left untouched: it
// is a plain tag/value/unit/timestamp record with no dependency on the retired
// service. It describes no connection and claims no source.
...
// The operational payload of one dashboard frame. It is generated by
// src/lib/industrial/simulator and is always delivered inside a
// `ClassifiedDashboardFrame` (src/lib/dashboard-demo/contract.ts), which states
// its classification, connection mode, scope, source, timestamps, quality and
// provenance. A bare snapshot carries no such claims and must never be rendered
// as operational data on its own. Text fields are message KEYS, not strings —
// the client translates, so FA/EN/DE stays complete.
