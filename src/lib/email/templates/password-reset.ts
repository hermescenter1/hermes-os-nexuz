...
import { PASSWORD_RESET_STRINGS, EMAIL_DIR, resolveEmailLocale } from "./email-locale";
      ...
      <tr>
        <td>
          <p style="color:#5a5a80;font-size:13px;margin:0 0 8px;line-height:1.6;">
            <strong style="color:#9d7fe8;">${s.securityLabel}</strong>
...
export function passwordResetEmailText(data: PasswordResetEmailData): string {
  const s = PASSWORD_RESET_STRINGS[resolveEmailLocale(data.locale)];
  return s.textBody
    .replace("{name}", data.name)
    .replace("{link}", data.resetLink)
