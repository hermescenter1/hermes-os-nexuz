...
export async function listConnectors(organizationId: string, opts?: {
  ...
  // PHASE 99 SECURITY — the site allow-list used to be skipped whenever a
  // gatewayId was supplied, so naming a gateway that belongs to another site in
  // the same organization returned that site's connector rows, whose `config`
  // carries OPC UA / MQTT endpoint and connection material. The gateway filter
  // and the site allow-list are independent predicates: apply BOTH.
