 ...
 * and — for infrastructure faults — the error CLASS recorded by
 * `logInfraFailure`.
 */
...
async function resolveFirstOrgId(userId: string, reqId: string): Promise<OrgResolution> {
  ...
  try {
    ...
    // and surfaced as "Authentication required". It is now recorded as what it
    // is — error CLASS and message only, never the query or any credential.
    logInfraFailure("database", `${AUTH_OPERATION}.resolve_organization`, err, reqId);
