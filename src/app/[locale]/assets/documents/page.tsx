 ...
 * of `MOCK_DOCUMENT_LINKS` — a module-level array in the repository — so a
...
import { getAssetsWithDocuments } from "@/lib/assets/db";
...
export default async function AssetsDocumentsPage() {
  ...
  let assets;
  try {
    assets = await getAssetsWithDocuments();
  } catch (err) {
    if (isDataScopeError(err)) return <DataUnavailableNotice code={err.code} />;
    throw err;
  }

  return <AssetDocumentsClient assets={assets} />;
}
