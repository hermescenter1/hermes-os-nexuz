...
// ── Icon definitions (presentation-only, no translatable content) ────────────
...
const CAPABILITY_ICONS: CapabilityIcon[] = [
  {
    key: "faultAnalysis",
    icon: (
...
export default async function DemoPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  // Build value propositions with i18n strings
  const valueProps = CAPABILITY_ICONS.map((cap) => ({
    key: cap.key,
    title: t(`capabilities.${cap.key}`),
    description: t(`capabilities.${cap.key}Desc`),
    icon: cap.icon,
  }));

  // Build trust items with i18n strings
  const trustKeys = ["built", "bilingual", "enterprise", "secure"] as const;
  ...
  return (
    // PHASE 104-I2 — /demo joins the public estate. It was rendering with no
    // public header, no primary navigation and no footer, which left the site's
    // main conversion target (linked from the header CTA on every other public
    // page) as a dead end with no way back into the site. `company` is the mode
    // PublicPageShell already documents for this surface, alongside About,
    // Contact and Careers.
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {valueProps.map((v) => (
                  <div key={v.key} className="flex gap-3 p-3.5 rounded-xl border border-[#1E2E40] bg-[#0C1420]/60">
                    ...
                    <div>
                      <p className="text-xs font-bold text-[#F0F4F8]">{v.title}</p>
                      <p className="text-[10px] text-[#5A6B80] mt-0.5 leading-relaxed">{v.description}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <ul className="space-y-2">
                {trustItems.map((item) => (
                  <li key={item.key} className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs text-[#8A9BB0]">{item.label}</span>
                  </li>
                ))}
              </ul>
            </div>
