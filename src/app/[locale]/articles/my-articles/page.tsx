...
const STATUS_STYLE: Record<string, { cls: string; dot: string }> = {
  ...
  REJECTED:  { cls: "bg-danger/10 text-danger border-danger/20",    dot: "bg-danger"  },
  ARCHIVED:  { cls: "bg-surface3 text-muted border-line/40",        dot: "bg-muted"   },
};

async function ArticleRow({ article, isFa, locale }: { article: ArticleListItem; isFa: boolean; locale: string }) {
  const t = await getTranslations("journalEditorial");
  const s = STATUS_STYLE[article.status] ?? STATUS_STYLE["DRAFT"];
  const statusLabel = t.has(`status.${article.status}`) ? t(`status.${article.status}`) : article.status;
  return (
          ...
          <div>
            ...
            {articles.length > 0 && (
              <p className="mt-1 text-sm text-muted">
                {articles.length} {t("myArticlesUnit")}
              </p>
            )}
          </div>
