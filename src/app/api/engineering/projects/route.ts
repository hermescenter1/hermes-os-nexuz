// PHASE 94B4 — engineering project list.
//
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  return withOtRoute(req, { permission: "view_engineering_project", bucket: "engineering-read" }, async (ctx) => {
    ...
    const res = await svc.repos.projects.listVisible(ctx, parseQuery(new URL(req.url)));
    if (!res.ok) return errorResponse(svcFail("INTERNAL_FAILURE"));
    return privateJson({
      ok: true,
      data: { items: res.value.items.map((p) => toEngineeringProjectSummaryDto(p as never)), total: res.value.total, take: res.value.take, skip: res.value.skip },
    });
  });
}
