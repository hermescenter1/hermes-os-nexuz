// PHASE 94B4 — project automation tag list.
//
...
import { toAutomationTagDto } from "@/lib/ot-edge/dto";
...
export async function GET(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  return withOtRoute(req, { permission: "view_engineering_project", bucket: "engineering-read" }, async (ctx) => {
    ...
    const res = await svc.repos.projects.listTags(ctx, id, parseQuery(new URL(req.url)));
    if (!res.ok) return errorResponse(svcFail("INTERNAL_FAILURE"));
    return privateJson({
      ok: true,
      data: { items: res.value.items.map((x) => toAutomationTagDto(x as never)), total: res.value.total, take: res.value.take, skip: res.value.skip },
    });
  });
}
