// PHASE 94B4 — project alarm definition list.
//
...
import { toAlarmDefinitionDto } from "@/lib/ot-edge/dto";
...
export async function GET(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  return withOtRoute(req, { permission: "view_engineering_project", bucket: "engineering-read" }, async (ctx) => {
    ...
    const res = await svc.repos.projects.listAlarms(ctx, id, parseQuery(new URL(req.url)));
    if (!res.ok) return errorResponse(svcFail("INTERNAL_FAILURE"));
    return privateJson({
      ok: true,
      data: { items: res.value.items.map((x) => toAlarmDefinitionDto(x as never)), total: res.value.total, take: res.value.take, skip: res.value.skip },
    });
  });
}
