 ...
 *
 * Phase 86C4B2B1D-SECURITY-5: the route is now authoring-gated (its
 * recommendation/playbook text embeds raw user-entered memory content), so the
 * default `beforeEach` establishes an authoring engineer; the anonymous and
 * non-authoring rejection is proven in dashboard-api-boundaries.test.ts.
 */
...
function seedProject(id: string, name = `Project ${id}`) {
  ...
  arr.push({ id, name, description: "", status: "active", userId: "u-test",
