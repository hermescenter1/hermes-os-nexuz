...
describe("87L.6F — full 5,136-leaf reconciliation (§2)", () => {
  ...
  it("classifies every leaf exactly once, with no duplicates", () => {
    ...
    // 94C1: +195 otEdge leaves (194 namespace + 1 appShell nav label) -> 5365
    // 94C2: +91 otEdge.onboarding leaves -> 5456
    // 92:   +35 observability leaves (SRE operator surface) -> 5491
    // 93:   observability namespace rebuilt as the SRE Control Room
    ...
    // Phase 96: +21 Pricing leaves (3 metadata.pages.pricing + 18 top-level pricing) -> 5613
    ...
    //            -> 6910  (B1.1: +1 careers.preferredExperience; B1.2: +4 locationTypeLabels + salaryPerYear; B1.3: +3 careers.apply.notAccepting{Title,Body,Cta}; B1.4: +2 careers.viewDetails/applicationsNotOpen − 1 retired careers.viewAndApply)
  // 109-C-UI.1: +36 automationStudio leaves (7237 -> 7273) for the TIA + SCADA/HMI engineering workspace: kinds (13 artifact-kind labels, previously the raw English union member rendered in all three locales), access (4 read/write/binding/alarm labels, same defect in the references list), artifact (12 - the new central surface for the fourteen artifacts with no textual source), bottom.changes, versions.locallyModified / .noLocalChanges, inspector.toggle, companion.artifacts / .artifactHint / .backToList. All 36 genuinely German, zero carryover; the identical-value allowlist is unchanged.
  // 109-C-UI.1 R1: +9 more automationStudio leaves (7273 -> 7282) closing the
  // Codex review's i18n finding: origins (5 - the whole DataOrigin union, whose
  // raw members `simulated` and `authored` were the visible text in every
  // locale), classification.label / .simulated (2 - the header showed the raw
  // SIMULATED token as its only visible statement), artifact.inspectSymbol and
  // inspector.close (2 - accessible names for the dossier's inspect controls
  // and the inspector drawer's close control). All 9 genuinely German; the
  // identical-value allowlist is unchanged.
  // LIVE-OPERATIONS x MAIN MERGE: 7455, MEASURED on the merged catalogue with
  // this file's own leaves() function, not arithmetic on the two pins. Both
  // sides moved this number off the 7282-leaf merge base (7edffafa): this
  // branch to 7355 (+73 for Live Operations) and main to 7382 (+100 for the
  // tenant-context copy and the SCADA Control Room). Adding the deltas is only
  // right if the two key sets are disjoint AND neither side removed a key, so
  // it was checked rather than assumed: this branch adds 73 and removes 0,
  // main adds 100 and removes 0, and the intersection of the two added sets
  // is EMPTY. The merged messages/en.json, fa.json and de.json each read 7455
  // leaves with zero asymmetric keys in any direction, and zero keys were lost
  // in the merge. No assertion was relaxed and no allowlist entry or TARGET
  // was added, removed or widened to reach this number.
  //
  // Both blocks below are kept unchanged as history. The first is this
  // branch's own Live Operations record; the second holds the PR-94 and PR-92
  // merge records, whose 7382 and 7302 were the measured truth for those
  // merges, not this one.
  // PHASE 109-C-UI.2 loop 3 (D-21): +1 liveOperations.filters.allAcknowledgements
  // (7354 -> 7355). The acknowledgement filter was accepted by the parser and
  // had no control, so it could only ever be set by a URL and never cleared.
  // PHASE 109-C-UI.2 loop 3: +1 appShell.nav.items.liveOperations (7353 ->
  // 7354). The page had no navigation entry at all, so it was reachable only
  // by typing the URL. DE 'Betriebsuebersicht' is the same word the page's own
  // title uses, so the label a reader clicks and the heading they land on are
  // one term, not two. Genuinely German in all three catalogues, zero
  // carryover, allowlist unchanged.
  // PHASE 109-C-UI.2: +71 liveOperations leaves (7282 -> 7353) for the Live
  // Operations surface (roadmap page 09): the advisory boundary (3), the
  // data-state (6) and freshness (4) vocabularies that let the page say
  // NOT_CONNECTED where a lesser screen would print 0, the degraded-source
  // banner (3), the operational summary (6), filters (3), severity (4),
  // time ranges (4), acknowledgement (3), origin (3), event fields (7),
  // three empty states (6), four capability disclosures (4) and five access
  // refusals (11), plus metaTitle/eyebrow/title/subtitle and the events
  // heading. All 71 genuinely German; the identical-value allowlist is
  // unchanged.
  // PR-94 MERGE: 7382, MEASURED on the merged catalogue in this commit, not
  // arithmetic on the two pins. Both sides moved this number off 7282 — this
  // branch to 7362 (+80 for the SCADA Control Room) and main to 7302 (+20 for
  // the tenant-context copy). Adding the deltas is only right if the two key
  // sets are disjoint AND neither side removed a key, so that was checked
  // rather than assumed: against the 7282-leaf merge base this branch adds 80
  // and removes 0, main adds 20 and removes 0, and the intersection of the two
  // added sets is EMPTY. The merged messages/en.json then reads 7382 leaves, and
  // de.json and fa.json read 7382 each with exact three-way key parity, which
  // the parity test below independently asserts. Zero keys were lost in the
  // merge. No assertion was relaxed and no allowlist entry was added, removed
  // or widened to reach this number.
  //
  // The block below is the record of the PREVIOUS merge and is kept unchanged
  // as history; its 7302 was the measured truth for that merge, not this one.
  // PR-92 MERGE: 7302, MEASURED on the merged catalogue, not arithmetic on the
  // two pins. Both sides moved this number off 7237 - this branch to 7257 (+20
  // for the tenant-context copy) and main to 7282 (+45 for 109-C-UI.1). Adding
  // the deltas is only right if the two key sets are disjoint AND neither side
  // removed a key, which is an assumption, so it was checked instead: against
  // the 7237-leaf merge base this branch adds 20 and removes 0, main adds 45
  // and removes 0, and the intersection of the two added sets is EMPTY. The
  // merged messages/en.json then reads 7302 leaves, and de.json and fa.json
  // read 7302 each with exact three-way key parity - which is what the parity
  // test below independently asserts. No assertion was relaxed and no
  // allowlist entry was added, removed or widened to reach this number.
    expect(allEn.length).toBe(8012);  // ATS-CANDIDATE-PRIVACY: +62 leaves, MEASURED (7950 -> 8012), all inside careers.privacy (the already-registered `careers` namespace): the candidate privacy notice (legal draft) - meta, draft banner, version line, intro, and its nine sections (applicant data, preliminary AI screening, human decision, purpose and legal basis, retention, rights, sharing and security, controller and contact, limitations and request route). Genuinely German in every leaf, zero German==English values, exact three-way key, placeholder and rich-tag parity; no allowlist entry.  // ATS-STAGE1-FORM: +40 leaves, MEASURED (7910 -> 7950), all inside careers.apply.form (the already-registered `careers` namespace): the Stage-1 public application form - processing note, section hints, the LinkedIn field, the three confirmations, the error summary (ICU plural), 11 field names, 9 field-error messages, the four refusal/unknown-outcome messages, and the received/reference copy. Genuinely German in every leaf, zero German==English values ("optional" is "freiwillig"), exact three-way key parity; no allowlist entry.  // ATS-M1 review fix: +2 leaves (7908 -> 7910) - ats.mgmt.form.copy.preferredExperience (the editor now round-trips preferred experience instead of wiping it) and ats.mgmt.settings.retention.approverOnly (only manage_retention may approve a retention policy). Both genuinely German.  // ATS-M1: +365 leaves, MEASURED (7543 -> 7908), all inside the already-registered `ats` namespace (de-catalog TRANSLATED_NS needs no new entry): ats.navSettings (1) and ats.mgmt (364) - the position list, lifecycle confirmations and results, publish-readiness vocabulary, refusal messages, the position editor (identity, public copy per locale, compensation, criteria, rubric dimensions, interview kit, assessment, owners, the 15 organization roles) and the six ATS Settings sections. Genuinely German in every leaf, zero German==English values, exact three-way key parity; no allowlist entry was added.  // ENGINEERING-HUB-TRILINGUAL: +79 engineeringHub leaves (7158 -> 7237) for the Executive Dashboard and the Engineering shell it renders in: nav (10 - the sidebar module list, the Modules heading, the back-to-site link and the two menu aria-labels), dashboard.metrics (6), .risk (14 incl. the four risk-level labels), .intelligence (17 incl. the agent and agent-status vocabularies), .benchmark (7), .domainHealth (10), .graph (9), .states (3), .source (2), plus lastUpdated / loadingMetrics / sourceLabel. One DE value is identical to EN by design - nav.dashboard, an established anglicism in German enterprise software, covered by PROTOCOL_OR_STANDARD here and recorded as accepted-german-loanword in de-identical-allowlist. nav.intelligence is NOT identical: German is Intelligenz, which is what this catalogue already uses for the same nav label elsewhere (nav.groups.intelligence). Every other leaf is genuinely German.  // 109-C-UI.3 review fix: +19 otEdge.controlRoom leaves (7343 -> 7362) — the alarm code column label, yes/no text alternatives for the acknowledgement glyph, two truncation notes, the engineering-permission note, and three enum vocabularies the alarm table used to print as raw tokens (severity 5, safetyClass 4, storedStatus 4). All genuinely German; "Informativ" and "Gemeldet: online" chosen precisely so no leaf is identical to English.  // 109-C-UI.3 nav: +1 engineeringHub.nav.controlRoom, the sidebar entry that makes the Control Room reachable. Genuinely German (Leitwarte).  // 109-C-UI.3: +60 otEdge.controlRoom leaves (7282 -> 7342) for the SCADA Control Room: the four-value connectivity vocabulary (4) and the seven reasons behind it (7), the provenance and uncertainty vocabularies (12) that carry where each value came from and how certain it is, the site/gateway surface (12), the declared protocol references (4) and configured alarm definitions (7), the three DATA_UNAVAILABLE explanations for capabilities this platform genuinely cannot serve (3), the five refusal messages (5), plus heading, subtitle, generatedAt, readOnlyNotice, eyebrow and metaTitle. All 60 genuinely German; otEdge is already in de-catalog TRANSLATED_NS and is held to zero carryover  // PR-92 MERGE: +45 from main (7257 -> 7302), the 109-C-UI.1 automationStudio leaves detailed above.  // PHASE-110-A1.0b R6.1: +2 leaves (7255 -> 7257): errors.resource.orgPrecondition{Title,Hint}, the copy for a 428 - a WRITE that named no organization at all. Deliberately NOT the orgConflict copy below, which says the page was showing a DIFFERENT organization: this page named none, and describing a switch the reader never made would be false in three languages.  // PHASE-110-A1.0b R3 (R3-2): +2 leaves (7253 -> 7255): errors.resource.orgConflict{Title,Hint}, the copy for a stale tenant precondition — a reader whose OTHER tab switched organization is told to reload, not to select an organization they already have.  // PHASE-110-A1.0b R3: +3 leaves, MEASURED (7250 -> 7253): orgAdministration.states.{sectionUnavailable,sectionForbidden,summaryIncomplete}, so the overview can say a section FAILED or is REFUSED instead of rendering it as an empty success. Genuinely German, no allowlist entry needed.  // PHASE-110-A1.0b: +13 leaves, MEASURED not hand-added (7237 -> 7250); the 13th is org.stateForbidden, added in R2 so the organization pages can say 'you lack the capability' instead of borrowing a context message. The organization switcher, nested inside the already-registered `appShell` and `errors` namespaces so de-catalog TRANSLATED_NS needs no new entry: appShell.shell.contextSelectionRequired (1) for the fourth chip state, appShell.organizationSwitcher (6: heading, switch, loading, saving, cancel, none) for the control itself, and errors.resource.orgSelection{Title,Hint} (2) for the 409 that says 'choose one' rather than 'you have none', and org.state{Unauthenticated,SelectionRequired,Unavailable} (3) so the five organization administration pages stop answering all four refusals with the single sentence org.noOrg. Genuinely German in every leaf, zero English carryover, no allowlist entry needed  // .  // ENGINEERING-HUB-TRILINGUAL: +79 engineeringHub leaves (7158 -> 7237) for the Executive Dashboard and the Engineering shell it renders in: nav (10 - the sidebar module list, the Modules heading, the back-to-site link and the two menu aria-labels), dashboard.metrics (6), .risk (14 incl. the four risk-level labels), .intelligence (17 incl. the agent and agent-status vocabularies), .benchmark (7), .domainHealth (10), .graph (9), .states (3), .source (2), plus lastUpdated / loadingMetrics / sourceLabel. One DE value is identical to EN by design - nav.dashboard, an established anglicism in German enterprise software, covered by PROTOCOL_OR_STANDARD here and recorded as accepted-german-loanword in de-identical-allowlist. nav.intelligence is NOT identical: German is Intelligenz, which is what this catalogue already uses for the same nav label elsewhere (nav.groups.intelligence). Every other leaf is genuinely German.  // IB-DASHBOARD-ALIGNMENT: +11 industrialBrain leaves for the redesigned Industrial Brain surface (7147 -> 7158): nav (7) is the local section navigation for a long analytical document; sections.primaryHypothesis and sections.alternativeHypotheses (2) because rank 1 is promoted out of the ranked list, which then keeps ranks 2+; workspace.awaitingHeading and awaitingBody (2) because the result region now exists before the first analysis, as the scroll and anchor target. All 11 genuinely German, zero carryover.  // 109-C1 R1.1: +2 automationStudio.bottom (runLabel, scopeNote).  // 109-C1: +187 automationStudio (Automation Engineering Studio, fully German, zero English carryover).  // D.0-R3 MAIN-COMPAT: +34 from current main (Phase 109-B0 disclosure copy). Re-derived from the merged catalogue: 6718 base + 34 main + 192 Phase 104. The two deltas are disjoint, which is itself evidence the key-level merge preserved both sides.  // 89A: +9 errors; 89C: +18 meta; 93B: +6 Copilot; 96: +21 pricing; 97: +74 complianceCenter; TRUST: +4; 102: +162 mediaHub + 1 nav; 103: +40 liveVoice; R2: +240 capability pages; R2-fix: +17 discovery paths; LEAD-REVIEW: +20 lead review; JOURNAL-LIFECYCLE: +24; JOURNAL-ENGAGEMENT: +24; JOURNAL-SAVE: +3; JOURNAL-TOMBSTONE: +1; MULTI-SITE-BOUNDARY: +12 multiSite; PHASE-107-JOURNAL-MOBILE-NAV: +2 journal.nav; PHASE-107-ENGINEERING-HUB: +7 engineeringHub; PHASE-107-PLATFORM-COMPONENTS: +5 dashboard.platformComponents; PHASE-107-STAGE-6A: +18 errors.resource; PHASE-107-STAGE-6A-OT-CONTEXT: +6 otEdge.states; PHASE-107-STAGE-6A-CONTEXT-UI: +4 errors.resource; PHASE-101-R: +60 industrialBrain.reference; 104-E/F: +85 observatory + journal pressroom; 104-I2: +67 demo/careers/vendors; 104-I3: +168 vendor directory/application + careers apply; 104-I3b: +12 vendor detail; 104-I3c: +38 demo form; 104-I3d: +2 ProvenExpert fallback; 104-I.D2: +54 dashboard.operations + dashboard.alarms (Gate A reference surfaces); 104-I.D3: +4 dashboard.operations.intelligenceTabs (Intelligence Wall internal tabs, previously hard-coded English in all three locales); B1-F01/F03: +91 (ats 69, customerSuccess 9, knowledgeGraph state vocabulary 13);
// B.2 (B0-F04): +3 — Engineering Knowledge Graph header (eyebrow/title/lede).
// B.1 R3: +27 more (ATS/Customer Success layout headers, enum labels resolved
// through the catalogue, LinkedIn/Indeed proper nouns).
// B.1 review round 2: +13 more (ats status labels 5, customerSuccess 8) for the
// residual hard-coded English the whole-page F03 check could not see.
  ...
  it("satisfies 7455 = translations + identicals + tokens + numeric/unit", () => {
    ...
    // ATS-CANDIDATE-PRIVACY: measured on the same catalogue as the assertion
    // above, so it pins the identical 8012 (+62 careers.privacy leaves, all
    // genuine translations).
    // ATS-STAGE1-FORM: measured on the same catalogue as the assertion above,
    // so it pins the identical 7950 (+40 careers.apply.form leaves, all genuine
    // translations).
    // ATS-M1: measured on the same catalogue as the assertion above, so it
    // pins the identical 7910 (+367 ats leaves, all genuine translations).
    // LIVE-OPERATIONS x MAIN MERGE: the bucket reconciliation is measured on the
    // same merged catalogue as the assertion above, so it pins the identical
    // 7455. The note below is the PR-94 record and is kept unchanged as history.
    // PR-94 MERGE: the bucket reconciliation is measured on the same merged
    // catalogue as the assertion above, so it pins the identical 7382. The
    // SCADA Control Room's leaves reconcile as translations, not as identicals
    // or tokens: every German value differs from its English source.
    ).toBe(8012);
